...
fn replace_once(src: &mut String, from: &str, to: &str, label: &str) {
    ...
    assert_eq!(n, 1, "PERF-022 transform expected exactly one {label}, found {n}");
    *src = src.replacen(from, to, 1);
}

fn replace_some(src: &mut String, from: &str, to: &str, min: usize, label: &str) {
    let n = src.matches(from).count();
    assert!(n >= min, "PERF-022 transform expected at least {min} {label}, found {n}");
...
fn main() {
    ...
    // PERF-022 keeps the proven PERF-015 monetary core and the PERF-021 certificate gate,
    // but replaces SHA-256 streaming content hashing with BLAKE3. This isolates whether
    // cryptographic batch-content hashing, rather than certificate verification, caused the
    // PERF-021 throughput drop.
    replace_once(
        &mut src,
    ...
    replace_some(&mut src, "_0014", "_0022", 3, "domain constants");
    replace_some(&mut src, "PERF-014", "PERF-022", 5, "PERF labels");
    replace_once(&mut src, "v1.4.0", "v2.2.0", "version label");
...
fn deterministic_cert_key() -> SigningKey {
    let mut seed = [0u8; 32];
    for i in 0..4u64 {
        let x = mix64(0xCE47_1F1C_CA11_0022u64 ^ i.wrapping_mul(0x9e37_79b9_7f4a_7c15));
...
fn build_batch_certs(
        ...
        for s in 0..streams {
            ...
            for tx in start..end {
                if tx as usize % streams == s {
                    let rec = canonical_record96(tx, arity, conflict_pct);
                    hasher.update(&rec);
                }
            }
    ...
    let old_flush = r#"                let flush_batch = |batch: usize,
                    ...
                    if done == expected_streams[batch] {
                        batch_ready_tx
                            .send(batch)
                            .expect("send PERF-022 batch-ready notification");
                    }
                };"#;
    ...
    let new_flush = r#"                let flush_batch = |batch: usize,
                                   ...
                                   cursors: &Vec<usize>,
                                   batch_starts: &mut Vec<usize>| {
                    // A stream-batch becomes executable only after its exact BLAKE3 content digest is certified.
                    let cert = batch_certs[batch * streams + s];
                    let vk = VerifyingKey::from_bytes(&cert.public)
                        .expect("PERF-022 invalid certificate public key");
                    let sig = Signature::from_bytes(&cert.signature);
                    vk.verify_strict(&cert_message(batch, s, &digest), &sig)
                        .expect("PERF-022 batch authorization certificate rejected");
