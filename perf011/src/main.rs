...
const VALUE: u64 = 100;
const MAGIC: u64 = 0xCA11_BAEB_0000_0011;
...
fn input_shard(serial: u64, shards: usize) -> usize {
    (mix64(serial ^ 0xA11C_E001_5EED_0011) as usize) & (shards - 1)
}
...
fn referenced_serial(tx: u64, k: usize, arity: usize, conflict_pct: u32) -> u64 {
    ...
    if conflict_pct > 0
        && tx > 0
        && k == 0
        && (mix64(tx ^ 0xC0AF_11C7_0000_0011) % 100) < conflict_pct as u64
    {
        (tx - 1) * arity as u64
    } else {
        own
...
fn output_shard(tx: u64, shards: usize) -> usize {
    (mix64(tx ^ 0x0A7B_17A5_F00D_0011) as usize) & (shards - 1)
}
...
fn run_case(
    ...
    // Canonical current state. Every shard has exactly one execution owner.
    ...
    // PERF-011 execution workers receive already-routed WorkItems. They no longer scan every
    // transaction, load owner masks, or reconstruct ids from wire metadata.
    ...
    for w in 0..workers {
        let (work_tx, work_rx) = mpsc::sync_channel::<Arc<BatchPacket>>(1);
        ...
        execution_handles.push(thread::spawn(move || {
            ...
            while let Ok(packet) = work_rx.recv() {
                ...
                for sb in &packet.stream_batches {
                    for item in &sb.by_worker[w] {
                        route_items_seen += 1;
                ...
                // All owners finish PREPARE before any owner decides commit/rollback.
                ...
                for &(slot, tx, cell) in &prepared {
                    ...
                    if eligible {
                        ...
                        panic!("PERF-011 rollback collision");
                    }
                }

                for tx in (packet.start_tx + w as u64..packet.end_tx).step_by(workers) {
                    let eligible =
                        prepared_counts[tx as usize].load(Ordering::Relaxed) as usize == arity;
                    if eligible {
                        ...
                        if outputs.insert(id, next).is_some() {
                            panic!("PERF-011 output collision");
                        }
                ...
                done_tx.send(w).expect("PERF-011 done channel closed");
            }
    ...
    // Receiver-local route batches are lock-free while being built. Each TCP receiver owns its
    // current by_worker vectors, then transfers the completed batch to the coordinator.
    let (route_tx, route_rx) = mpsc::sync_channel::<StreamBatch>((streams * 4).max(4));

    let listener = TcpListener::bind("127.0.0.1:0").expect("bind PERF-011 loopback listener");
    let addr = listener.local_addr().expect("PERF-011 listener address");
    ...
    let receiver_route_tx = route_tx.clone();
    ...
    let receiver_manager = thread::spawn(move || {
        ...
        for _ in 0..streams {
            let (mut stream, _) = listener.accept().expect("accept PERF-011 loopback stream");
            stream.set_nodelay(true).expect("receiver TCP_NODELAY");

            let mut hello = [0u8; 8];
            stream.read_exact(&mut hello).expect("read PERF-011 stream id");
            let s = u64::from_le_bytes(hello) as usize;
            assert!(s < streams, "invalid PERF-011 stream id");
            assert!(!seen[s], "duplicate PERF-011 stream id");
            seen[s] = true;

            let ready = Arc::clone(&receiver_ready);
            let go = Arc::clone(&receiver_go);
            let route_tx = receiver_route_tx.clone();
            ...
            receivers.push(thread::spawn(move || {
                ...
                let flush_batch = |batch: usize, by_worker: &mut Vec<Vec<WorkItem>>| {
                    let moved: Vec<Vec<WorkItem>> = by_worker
                        .iter_mut()
                        .map(|v| std::mem::take(v))
                        .collect();
                    route_tx
                        .send(StreamBatch { batch, stream: s, by_worker: moved })
                        .expect("send PERF-011 routed stream batch");
                    for v in by_worker.iter_mut() {
                        v.reserve((batch_size * arity / workers / streams).max(16));
                    }
                };
                ...
                for i in 0..expected {
                    reader.read_exact(&mut record).expect("read complete PERF-011 record");
                    ...
                    assert_eq!(tx, expected_tx, "PERF-011 wire tx ordering mismatch");
                    ...
                    assert_eq!(declared_arity, arity, "PERF-011 wire arity mismatch");
                    assert_eq!(declared_value, arity as u64 * VALUE, "PERF-011 declared value mismatch");
                    ...
                    assert_eq!(tag, wire_tag(tx, arity, declared_value, &inputs), "PERF-011 integrity mismatch");
        ...
        let mut routed_inputs = 0u64;
        for h in receivers {
            let (c, r) = h.join().expect("PERF-011 receiver worker panic");
            checksum ^= c;
            routed_inputs += r;
        }
        (checksum, routed_inputs)
    ...
    for s in 0..streams {
        ...
        senders.push(thread::spawn(move || {
            let mut stream = TcpStream::connect(addr).expect("connect PERF-011 loopback stream");
            stream.set_nodelay(true).expect("sender TCP_NODELAY");
            stream
                .write_all(&(s as u64).to_le_bytes())
                .expect("write PERF-011 stream id");
            ...
            for i in 0..count {
                ...
                writer.write_all(&record).expect("write complete PERF-011 record");
            }
            writer.flush().expect("flush PERF-011 stream");
        }));
    }

    drop(route_tx);

    let expected_streams: Vec<usize> = (0..batches)
        .map(|b| {
    ...
    let mut pending: Vec<Vec<Option<StreamBatch>>> = (0..batches)
    ...
    for b in 0..batches {
        while pending[b].iter().filter(|x| x.is_some()).count() < expected_streams[b] {
            let sb = route_rx.recv().expect("PERF-011 route channel closed early");
            let sb_batch = sb.batch;
            let sb_stream = sb.stream;
            assert!(sb_batch < batches && sb_stream < streams);
            assert!(pending[sb_batch][sb_stream].is_none(), "duplicate PERF-011 stream batch");
            pending[sb_batch][sb_stream] = Some(sb);
        }

        let mut stream_batches = Vec::with_capacity(expected_streams[b]);
        for s in 0..streams {
            if let Some(sb) = pending[b][s].take() {
                stream_batches.push(sb);
            }
        }
        assert_eq!(stream_batches.len(), expected_streams[b]);

        let start_tx = (b * batch_size) as u64;
        let end_tx = ((b + 1) * batch_size).min(txs as usize) as u64;
        let packet = Arc::new(BatchPacket { start_tx, end_tx, stream_batches });

        let exec_start = Instant::now();
        for tx in &work_txs {
            tx.send(Arc::clone(&packet)).expect("PERF-011 execution channel closed");
        }
        for _ in 0..workers {
            done_rx.recv().expect("PERF-011 done channel closed early");
        }
        batch_exec_latencies.push(exec_start.elapsed());
    }

    for h in senders {
        h.join().expect("PERF-011 sender worker panic");
    }
    let (wire_checksum, routed_inputs) = receiver_manager
        .join()
        .expect("PERF-011 receiver manager panic");
    ...
    for h in execution_handles {
        let (c, a, sum, rc, rv, oc, ov, routes) =
            h.join().expect("PERF-011 execution worker panic");
        committed += c;
        aborted += a;
        state_checksum ^= sum;
        remaining_cells += rc;
...
fn main() {
    ...
    let workers = parse_list(&args, "--workers", vec![6usize, 8]);
    let shards = parse_list(&args, "--shards", vec![1024usize]);
    let conflicts = parse_list(&args, "--conflicts", vec![0u32, 10]);
    let batches = parse_list(&args, "--batches", vec![2048usize, 4096, 8192]);
    let streams = parse_list(&args, "--streams", vec![1usize, 2, 4]);
    let sizes = parse_list(&args, "--sizes", vec![128usize]);

    println!("CALIBRE GEN2 PERF-011 v1.1.0");
    println!("DIRECT RECEIVER-ROUTED BATCH INBOX -> ONE-PREPARE-BARRIER ATOMIC ENGINE");
    println!("Security: OFF | Consensus: OFF | Persistence: OFF");
    println!("Change vs PERF-010: TCP receivers emit exact owner-local WorkItems; no owner-mask/global wire scan");
    println!("Execution coordination: one PREPARE barrier per batch; coordinator completion provides batch fencing");
    println!("Sender serialization remains inside timing so comparison with PERF-010 is conservative");
    println!("p95 below is batch-ready-to-all-workers-done latency, NOT end-to-end/WAN finality latency");
    println!();
    ...
    if let Some(r) = rows.iter().find(|r| {
        ...
        println!(
            "SAME CONFIG AS PERF-010 / 8W / 4S / B4096: {:.0} committed tx/s | p95={:.1}us",
            r.commit_tps, r.p95_us
        );
        println!(
            "SAME-CONFIG >4M TARGET: {}",
            if r.commit_tps >= 4_000_000.0 { "PASS" } else { "NOT YET" }
        );
        println!(
            "SAME-CONFIG >5M RESEARCH TARGET: {}",
    ...
    println!("WIRE RECORDS DIRECTLY ROUTED TO OWNER-LOCAL EXECUTION INBOXES: YES");
...
mod tests {
    ...
    #[test]
    fn direct_routed_no_conflict_preserves_value() {
    ...
    #[test]
    fn direct_routed_conflict_rolls_back() {
