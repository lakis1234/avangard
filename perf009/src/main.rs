...
const VALUE: u64 = 100;
const MAGIC: u64 = 0xCA11_BAE9_0000_0009;
...
fn input_shard(serial: u64, shards: usize) -> usize {
    (mix64(serial ^ 0xA11C_E001_5EED_0009) as usize) & (shards - 1)
}
...
fn referenced_serial(tx: u64, k: usize, arity: usize, conflict_pct: u32) -> u64 {
    ...
    if conflict_pct > 0
        && tx > 0
        && k == 0
        && (mix64(tx ^ 0xC0AF_11C7_0000_0009) % 100) < conflict_pct as u64
    {
        (tx - 1) * arity as u64
    } else {
        own
...
fn output_shard(tx: u64, shards: usize) -> usize {
    (mix64(tx ^ 0x0A7B_17A5_F00D_0009) as usize) & (shards - 1)
}
...
struct WireSlot {
    inputs: [AtomicU64; MAX_ARITY],
}

impl WireSlot {
    fn new() -> Self {
        Self {
            inputs: std::array::from_fn(|_| AtomicU64::new(u64::MAX)),
        }
    }
}
...
fn run_case(
    ...
    // Build canonical current state before timing. Every shard has exactly one execution owner.
    ...
    // Wire-decoded inputs are the execution source. No pre-routed synthetic tx list is used.
    let wire_slots: Arc<Vec<WireSlot>> =
        Arc::new((0..txs).map(|_| WireSlot::new()).collect());
    ...
    // One completion counter per microbatch. A receiver marks a batch only after all records
    // belonging to its TCP stream in that batch were parsed and stored into wire_slots.
    ...
    // Execution workers use the PERF-007 three-point fused microbatch protocol.
    ...
    for w in 0..workers {
        ...
        execution_handles.push(thread::spawn(move || {
            ...
            for b in 0..batches {
                ...
                // Main releases this only when every wire record in the batch has arrived.
                phase.wait();

                // Direct wire-fed prepare. Each worker scans the received input references in
                // deterministic tx order but mutates only the shards it owns.
                for tx in start_tx as u64..end_tx as u64 {
                    for k in 0..arity {
                        let serial = wire_slots[tx as usize].inputs[k].load(Ordering::Relaxed);
                        assert_ne!(serial, u64::MAX, "execution saw an unparsed wire input");
                        let sid = input_shard(serial, shards);
                        if sid % workers != w {
                            continue;
                        }
                ...
                // Fused commit/rollback plus independent output creation.
                for &(slot, tx, cell) in &prepared {
                    ...
                    if eligible {
                        ...
                        panic!("PERF-009 rollback collision");
                    }
                }
                ...
                for tx in (start_tx as u64 + w as u64..end_tx as u64).step_by(workers) {
                    ...
                    if eligible {
                        ...
                        if outputs.insert(id, next).is_some() {
                            panic!("PERF-009 output collision");
                        }
    ...
    // Real TCP loopback ingress. Connection setup and stream-ID handshakes are excluded from timing.
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind PERF-009 loopback listener");
    let addr = listener.local_addr().expect("PERF-009 listener address");
    ...
    let receiver_manager = thread::spawn(move || {
        ...
        for _ in 0..streams {
            let (mut stream, _) = listener.accept().expect("accept PERF-009 loopback stream");
            stream.set_nodelay(true).expect("receiver TCP_NODELAY");

            let mut hello = [0u8; 8];
            stream.read_exact(&mut hello).expect("read PERF-009 stream id");
            let s = u64::from_le_bytes(hello) as usize;
            assert!(s < streams, "invalid PERF-009 stream id");
            assert!(!seen[s], "duplicate PERF-009 stream id");
            seen[s] = true;
            ...
            receivers.push(thread::spawn(move || {
                ...
                let mark_done = |b: usize| {
                    ...
                    if done == expected_streams[b] {
                        batch_tx.send(b).expect("send PERF-009 batch-ready notification");
                    }
                };
                ...
                for i in 0..expected {
                    reader.read_exact(&mut record).expect("read complete PERF-009 record");
                    ...
                    assert_eq!(tx, expected_tx, "PERF-009 wire tx ordering mismatch");
                    ...
                    assert_eq!(declared_arity, arity, "PERF-009 wire arity mismatch");
                    assert_eq!(declared_value, arity as u64 * VALUE, "PERF-009 declared value mismatch");

                    let mut inputs = [u64::MAX; MAX_ARITY];
                    for k in 0..arity {
                        let off = 16 + k * 8;
                        let serial = u64::from_le_bytes(record[off..off + 8].try_into().unwrap());
                        inputs[k] = serial;
                    }
                    assert_eq!(tag, wire_tag(tx, arity, declared_value, &inputs), "PERF-009 integrity mismatch");

                    for (k, serial) in inputs.iter().take(arity).enumerate() {
                        wire_slots[tx as usize].inputs[k].store(*serial, Ordering::Relaxed);
                    }
        ...
        let mut checksum = 0u64;
        for h in receivers {
            checksum ^= h.join().expect("PERF-009 receiver worker panic");
        }
        checksum
    });
    ...
    for s in 0..streams {
        ...
        senders.push(thread::spawn(move || {
            let mut stream = TcpStream::connect(addr).expect("connect PERF-009 loopback stream");
            stream.set_nodelay(true).expect("sender TCP_NODELAY");
            stream
                .write_all(&(s as u64).to_le_bytes())
                .expect("write PERF-009 stream id");
            ...
            for i in 0..count {
                ...
                writer.write_all(&record).expect("write complete PERF-009 record");
            }
            writer.flush().expect("flush PERF-009 stream");
        }));
    }
    ...
    for b in 0..batches {
        while !batch_ready_flags[b] {
            let ready_batch = batch_ready_rx
                .recv()
                .expect("PERF-009 batch-ready channel closed early");
    ...
    for h in senders {
        h.join().expect("PERF-009 sender worker panic");
    }
    let wire_checksum = receiver_manager
        .join()
        .expect("PERF-009 receiver manager panic");
    ...
    for h in execution_handles {
        let (c, a, sum, rc, rv, oc, ov) = h.join().expect("PERF-009 execution worker panic");
        committed += c;
        aborted += a;
        state_checksum ^= sum;
        remaining_cells += rc;
...
fn main() {
    ...
    let streams = parse_list(&args, "--streams", vec![1usize, 4, 8]);
    let sizes = parse_list(&args, "--sizes", vec![128usize]);

    println!("CALIBRE GEN2 PERF-009 v0.9.0");
    println!("INTEGRATED TCP LOOPBACK -> WIRE PARSE -> FUSED SHARDED STATE EXECUTION");
    println!("Security: OFF | Consensus: OFF | Persistence: OFF");
    println!("Wire records carry the actual input references consumed by the execution engine.");
    println!("Transport and execution overlap by microbatch; state construction is outside timing.");
    println!("This is LOCAL LOOPBACK end-to-end execution, NOT distributed network TPS.");
    println!();
    ...
    let best = rows
        ...
        .expect("PERF-009 results");

    println!("=== DECISION ===");
    println!(
        "BEST NO-CONFLICT INTEGRATED COMMIT TPS: {:.0} | {}B | {} streams | batch={} | p95={:.1}us",
        best.commit_tps, best.record_size, best.streams, best.batch_size, best.p95_us
    );
    ...
    if let Some(r) = rows.iter().find(|r| {
        ...
        println!(
            "1M / 128B / 4 STREAMS / BATCH4096: {:.0} committed tx/s | p95={:.1}us",
            r.commit_tps, r.p95_us
        );
        println!(
            "INTEGRATED >3M AND p95<=2ms TARGET: {}",
            if r.commit_tps >= 3_000_000.0 && r.p95_us <= 2_000.0 {
                "PASS"
            } else {
                "NOT YET"
            }
    ...
    if let Some(r) = rows.iter().find(|r| {
        ...
        println!(
            "10%% CONFLICT / 128B / 4 STREAMS / BATCH4096: {:.0} committed tx/s | p95={:.1}us | aborted={}",
            r.commit_tps, r.p95_us, r.aborted
        );
    }

    println!("WIRE INPUTS ACTUALLY FEED STATE EXECUTION: YES");
...
mod tests {
    ...
    #[test]
    fn integrated_wire_to_state_no_conflict() {
    ...
    #[test]
    fn integrated_wire_to_state_conflict_rolls_back() {
